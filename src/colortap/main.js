import { ColorTapGame } from "./colorTapGame.js";
import { bindUiTapSounds, createGameAudio } from "../shared/audio.js";

const PROGRESS_KEY = "color-tap-progress";
const SETTINGS_KEY = "color-tap-settings";
const GLOBAL_SETTINGS_KEY = "kids-global-settings";

const game = new ColorTapGame();
...
const progressState = { bestStars: 0 };
...
function loadProgress() {
  try {
    ...
    progressState.bestStars = Number(parsed.bestStars || 0);
  } catch {
    progressState.bestStars = 0;
  }
}
...
function renderCards(snapshot) {
  board.innerHTML = snapshot.cards.map((card) => `
    <button class="card ${snapshot.lastTappedId === card.id ? "is-active" : ""}" type="button" data-color-id="${card.id}" style="--accent:${card.accent}">
...
function render(snapshot) {
  targetBadge.innerHTML = `<span class="target-icon" style="color:${snapshot.target.accent}">${snapshot.target.icon}</span><span>${snapshot.target.name}</span>`;
  guideText.textContent = `点 ${snapshot.target.name}`;
  starsValue.textContent = String(snapshot.stars);
  tapsValue.textContent = String(snapshot.totalTaps);
  bestValue.textContent = String(progressState.bestStars);
...
board.addEventListener("click", (event) => {
  ...
  const snapshot = game.tapColor(button.dataset.colorId);
  progressState.bestStars = Math.max(progressState.bestStars, snapshot.stars);
  saveProgress();
  render(snapshot);
  if (snapshot.success) {
    gameAudio.success();
...
resetBtn.addEventListener("click", () => {
  ...
  const targetName = snapshot.target?.name || "颜色";
