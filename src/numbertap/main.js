import { NumberTapGame } from "./numberTapGame.js";
import { bindUiTapSounds, createGameAudio } from "../shared/audio.js";

const PROGRESS_KEY = "numbertap-game-progress";
const SETTINGS_KEY = "numbertap-game-settings";
const GLOBAL_SETTINGS_KEY = "kids-global-settings";

const game = new NumberTapGame();
...
function renderCards(snapshot) {
  board.innerHTML = snapshot.cards.map((card) => `
    <button class="card ${snapshot.lastTappedId === card.id ? "is-active" : ""}" type="button" data-number-id="${card.id}" style="--accent:${card.accent}">
      <span class="card-num">${card.display}</span>
      <span class="card-label">${card.chineseNum}</span>
    </button>
  `).join("");
}

function render(snapshot) {
  targetBadge.innerHTML = `<span class="target-num" style="color:${snapshot.target.accent}">${snapshot.target.display}</span><span class="target-label">${snapshot.target.chineseNum}</span>`;
  guideText.textContent = `找 ${snapshot.target.display}（${snapshot.target.chineseNum}）`;
...
board.addEventListener("click", (event) => {
  ...
  const snapshot = game.tapNumber(button.dataset.numberId);
  ...
  if (snapshot.success) {
    ...
    showCelebration(`找到 ${snapshot.tapped.display} 啦`);
...
resetBtn.addEventListener("click", () => {
  ...
  speak(`重新来，找${snapshot.target.name}。`, true);
});
